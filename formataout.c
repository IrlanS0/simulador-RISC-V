    ...
    while (sim->run) {
        ...
        switch (opcode) {
            ...
            case 0b0110011: {
                ...
                } else if (funct7 == 0b0100000) { // RV32I (sub/sra)
                    if (funct3 == 0b000) { // sub
                        const uint32_t data = sim->x[rs1] - sim->x[rs2];
                        sprintf(col2_inst, "%s,%s,%s", sim->x_label[rd], sim->x_label[rs1], sim->x_label[rs2]);
                        sprintf(col3_details, "%s=0x%08x-0x%08x=0x%08x", sim->x_label[rd], sim->x[rs1], sim->x[rs2], data);
                ...
                } else if (funct7 == 0b0000001) { // RV32M
                    if (funct3 == 0b000) { // mul
                        ...
                        uint32_t data;
                        if (divisor == 0) data = 0xFFFFFFFF; // Divisão por zero
                        else if (dividend == 0x80000000 && divisor == -1) data = 0x80000000; // Overflow
                        else data = (uint32_t)(dividend / divisor);
                        sprintf(col2_inst, "%s,%s,%s", sim->x_label[rd], sim->x_label[rs1], sim->x_label[rs2]);
                        sprintf(col3_details, "%s=0x%08x/0x%08x=0x%08x", sim->x_label[rd], sim->x[rs1], sim->x[rs2], data);
                        ...
                        const int32_t divisor = (int32_t)sim->x[rs2];
                        uint32_t data;
                        if (divisor == 0) data = (uint32_t)dividend; // Resto da divisão por zero
                        else if (dividend == 0x80000000 && divisor == -1) data = 0; // Overflow
                        else data = (uint32_t)(dividend % divisor);
                        sprintf(col2_inst, "%s,%s,%s", sim->x_label[rd], sim->x_label[rs1], sim->x_label[rs2]);
                        sprintf(col3_details, "%s=0x%08x%%%0x%08x=0x%08x", sim->x_label[rd], sim->x[rs1], sim->x[rs2], data);
            ...
            case 0b0010011: {
                ...
                if (funct3 == 0b000) { // addi
                    ...
                    write_reg(sim, rd, data);
                } else if (funct3 == 0b001) { // slli (RV32I)
                    ...
                    write_reg(sim, rd, data);
                } else if (funct3 == 0b101) { // srli/srai (RV32I)
            ...
            // --- Sistema (EBREAK) ---
            case 0b1110011: {
                if (funct3 == 0b000 && imm_i == 1) { // ebreak
...
int main(int argc, char *argv[]) {
    ...
    sim.pc = sim.offset;
    sim.output = output;
    sim.x_label[0] = "zero"; sim.x_label[1] = "ra"; sim.x_label[2] = "sp"; 
    sim.x_label[3] = "gp"; sim.x_label[4] = "tp"; sim.x_label[5] = "t0"; 
    sim.x_label[6] = "t1"; sim.x_label[7] = "t2"; sim.x_label[8] = "s0"; 
    sim.x_label[9] = "s1"; sim.x_label[10] = "a0"; sim.x_label[11] = "a1"; 
    sim.x_label[12] = "a2"; sim.x_label[13] = "a3"; sim.x_label[14] = "a4"; 
    sim.x_label[15] = "a5"; sim.x_label[16] = "a6"; sim.x_label[17] = "a7"; 
    sim.x_label[18] = "s2"; sim.x_label[19] = "s3"; sim.x_label[20] = "s4"; 
    sim.x_label[21] = "s5"; sim.x_label[22] = "s6"; sim.x_label[23] = "s7"; 
    sim.x_label[24] = "s8"; sim.x_label[25] = "s9"; sim.x_label[26] = "s10"; 
    sim.x_label[27] = "s11"; sim.x_label[28] = "t3"; sim.x_label[29] = "t4"; 
    sim.x_label[30] = "t5"; sim.x_label[31] = "t6";
